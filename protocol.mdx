---
title: Measurement, Reporting, and Verification (MRV) Protocol for OAE Carbon Removals
version: '2.0'
...
$$
\tag{Full Equation} \text{Mg}^{2+} + 2\text{OH}^- + 1.65(\text{CO}_{2aq}) \rightarrow \text{Mg}^{2+} + 1.48(\text{HCO}_3^-) + 0.17(\text{CO}_3^{2-}) + 0.18(\text{OH}^-)
$$

Coefficients vary with pH, T, S and P. Values shown for pH = 8.1, 20°C, 35ppt, 10dB

$$
\tag{Simplified Equation} \text{OH}^- + \text{CO}_2 \rightarrow \text{HCO}_3^-
$$
...
$$
\tag{4} CDR_{ww} = (p\text{CO}_{2_{pre}} - p\text{CO}_{2_{post}}) \times k_{CO2} \times q_{ww} \times t
$$
...
$$
\text{Ca}^{2+} + 2\text{HCO}_3^- \rightarrow \text{CO}_{2g} + \text{CaCO}_{3s} + \text{H}_2\text{O}
$$
...
Mathesius, S., Hofmann, M., Caldeira, K. & Schellnhuber, H. J. 2015. Long-term response of oceans to CO₂ removal from the atmosphere. Nat. Clim. Change 5, 1107--1114.
...
$$
\tag{A1.1} \text{Mg}^{2+} + 2\text{OH}^- + A(\text{CO}_{2aq}) \rightarrow \\ \text{Mg}^{2+} + B(\text{HCO}_3^-) + C(\text{CO}_3^{2-}) + D(\text{OH}^-)
$$
...
$$
\tag{A1.2} Mg(OH)_2 + 2\text{CO}_2 \rightarrow \text{Mg}^{2+} + 2\text{HCO}_3^-
$$

Conversely, _A_ approaches 1 as pH rises and the dominant reaction becomes;

$$
\tag{A1.3} Mg(OH)_2 + \text{CO}_2 \rightarrow \text{Mg}^{2+} + \text{CO}_3^{2-}
$$
...
2.  The sinking of any solid, undissolved alkalinity out of the surface mixed layer such that dissolved alkalinity generation, CO₂ undersaturation, air contacting and CDR does not occur within the time period.
    ...
    <tr>
      ...
      <td>
        <ul>
          ...
          <li>Effluent pCO₂ baseline completed</li>
      ...
      <td>
        <ul>
          ...
          <li>
            ...
            deployments of ocean sensors) - pH, temperature, salinity, pH, pCO₂
